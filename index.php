...
<div class="productosCategorias">
  ...
  <hr>
  <?php
  $categoriaSeleccionada = 'Fútbol'; 

  $sentenciaSQL = $conexion->prepare("SELECT * FROM Productos INNER JOIN Categorias ON Productos.Categorias_ID_Categoria=Categorias.ID_Categoria WHERE Categorias.Nombre_Categoria=:categoria LIMIT 10");
  $sentenciaSQL->bindParam(":categoria", $categoriaSeleccionada);
  $sentenciaSQL->execute();
  $listaProductos = $sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);
  ?>
  <div class="">
  ...
  <?php
  $categoriaSeleccionada = 'Caza y Pesca'; 
