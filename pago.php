...
if ($_POST) {
    if (
        preg_match('/^[0-9]+$/', $_SESSION['ID_Usuario']) &&
        preg_match('/^[0-9,.]+$/', $_SESSION['valorIVA']) &&
        preg_match('/^[0-9,.]+$/', $_SESSION['totalConIVA'])
    ) {
        ...
        $sentenciaSQL->bindParam(":IdMetodoPago", $_POST['metodoPago']);
        $sentenciaSQL->execute();
